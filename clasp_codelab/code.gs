...
function 建立折線圖() {
  try {
    ...
    var chart = sheet.newChart()
      .setChartType(Charts.ChartType.COMBO)            // 複合圖表 (Combo Chart)
      .addRange(資料範圍)
      .setNumHeaders(1)                                // 確保第一列作為標題欄位
      .setPosition(15, 1, 0, 0)
      .setOption("title", "2026 年月度營收趨勢與利潤")
      .setOption("titleTextStyle", { fontSize: 16, bold: true, color: "#1a237e" })
      .setOption("width", 800)
      .setOption("height", 450)
      .setOption("seriesType", "line")                 // 預設所有系列為折線圖
      .setOption("series", {
        2: { type: "bars" }                            // 將第 3 個系列（利潤，索引為 2）設定為柱狀圖
      })
      ...
      .setOption("colors", ["#1a73e8", "#ea4335", "#34a853"]) // 藍色（營收）、紅色（成本）、綠色（利潤）
      .build();

    sheet.insertChart(chart);

    Logger.log("✅ 複合圖表（折線圖+利潤柱狀圖）已建立！");
    SpreadsheetApp.getUi().alert("✅ 月度趨勢複合圖表（折線+利潤柱狀）已生成！");
